...
DEFINE('ALOHA_OPTION_TREE_NOTICE', __('You can safely deactivate & uninstall the OptionTree plugin. ', ALOHA_DOMAIN));
...
function aloha_option_tree_message() {
    if (is_plugin_active(ALOHA_OPTION_TREE_PLUGIN) || file_exists(WP_PLUGIN_DIR . '/' . ALOHA_OPTION_TREE_PLUGIN)) {
        ...
        printf('<div class="%1$s" data-dismissible="%3$s"><p>%2$s</p></div>', esc_attr($class), ALOHA_OPTION_TREE_NOTICE, $key);
    }
}
...
function aloha_init() {
    if (!is_plugin_active(ALOHA_WIDGET_PACK_PLUGIN)) {
        ...
        //@todo better to load translations in the init hook and later in the execution. 
