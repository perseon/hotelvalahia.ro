...
//                        }
        ...
        private function _importElementorContentImages() {
            ...
            $sql_ready = $wpdb->prepare($sql);
            $results = $wpdb->get_results($sql_ready);
...
//                        }
        ...
        public function _content_install_settings() {
            ...
            $mphb_search_results_page = get_page_by_title( 'Availability Search' );
...
//            error_log('importing '.$metafield);
            global $wpdb;
            $sql = "SELECT p.ID, pm.meta_value FROM {$wpdb->prefix}posts p JOIN {$wpdb->prefix}postmeta pm ON pm.post_id=p.ID WHERE p.post_status = 'publish' AND pm.meta_key='".$metafield."'  AND pm.meta_value LIKE '%%s%' ";
            $search_term = 'import.bellevuetheme.com';
            $sql_ready = $wpdb->prepare($sql,$wpdb->esc_like( $search_term )); 
            $results = $wpdb->get_results($sql_ready);
            foreach ($results as $result) {
                $post_id = $result->ID;
